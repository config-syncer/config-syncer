...
type SearchResultLister interface {
	...
	List(selector labels.Selector) (ret []*kubed.SearchResult, err error)
...
// List lists all SearchResults in the indexer.
func (s *searchResultLister) List(selector labels.Selector) (ret []*kubed.SearchResult, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kubed.SearchResult))
	})
	return ret, err
}
...
type SearchResultNamespaceLister interface {
	...
	Get(name string) (*kubed.SearchResult, error)
...
// List lists all SearchResults in the indexer for a given namespace.
func (s searchResultNamespaceLister) List(selector labels.Selector) (ret []*kubed.SearchResult, err error) {
...
// Get retrieves the SearchResult from the indexer for a given namespace and name.
func (s searchResultNamespaceLister) Get(name string) (*kubed.SearchResult, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(kubed.Resource("searchresult"), name)
	}
	return obj.(*kubed.SearchResult), nil
}
